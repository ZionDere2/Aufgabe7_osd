...
static Layer exercise5recipe1[1] = {
    ActualPancake,
};
static Layer exercise5recipe2[2] = {
    Fruit,
    Cream,
};
static Layer exercise5recipe3[4] = {
    ActualPancake,
    Fruit,
    Cream,
    ActualPancake,
};
static Layer exercise5recipe4[6] = {
    ActualPancake,
    ActualPancake,
    ActualPancake,
    ActualPancake,
...
static Layer exercise5recipe5[8] = {
    ActualPancake,
    Cream,
    ActualPancake,
    Fruit,
...
static Layer exercise5recipe6[3] = {
    ActualPancake,
    ActualPancake,
    ActualPancake,
};
static Layer exercise5recipe7[11] = {
    Fruit,
    Fruit,
    Fruit,
    Fruit,
...
{
    int result = 1;
    if ((fabsf(expected.r - got.r) >= 0.0001) || (fabsf(expected.g - got.g) >= 0.0001) || (fabsf(expected.b - got.b) >= 0.0001))
    {
        result = 0;
    }
    return result;
...
char *render_exercises_23(RGB expected, RGB got, bool X_or_Y)
{
    ...
    snprintf(context, len, "Wir testen %s:\n" TESTRUNNER_LABEL "Erwartet: R = %.4f G = %.4f B = %.4f\n" TESTRUNNER_LABEL "Erhielt:  R = %.4f G = %.4f B = %.4f\n", X_or_Y ? "x" : "y", expected.r,
             expected.g, expected.b, got.r, got.g, got.b);

    return context;
}
char *render_rgbs(RGB x, RGB y)
{

    int len = strlen(TESTRUNNER_LABEL "  x:\nR = ") + 7 + strlen(" G = ") + 7 +
              strlen(" B = ") + 7 + strlen("\n") + strlen(TESTRUNNER_LABEL "  y:\nR = ") + 7 +
              strlen(" G = ") + 7 + strlen(" B = ") + 7 + strlen("\n") + 1;
    // The + 7 results from each float in the RGB struct being printed to four decimal points, potentially with a - sign
    char *context = mallocx(len); // size
    snprintf(context, len, TESTRUNNER_LABEL "  x:\nR = %.4f G = %.4f B = %.4f\n" TESTRUNNER_LABEL "  y:\nR = %.4f G = %.4f B = %.4f\n", x.r,
             x.g, x.b, y.r, y.g, y.b);
...
{
    const char *x = format_int(testcase.x);
    int len = strlen("Wir berechnen den frumpleflabblefizzledipp von x = .") + strlen(x) + 1;
    char *str = mallocx(len);
    snprintf(str, len, "Wir berechnen den frumpleflabblefizzledipp von x = %s.", x);
    free((char *)x);
    return str;
}
char *render_exercises_5(exercises_5_data testcase)
{
    ...
    for (int i = 0; i < testcase.number_of_layers; i++)
    {
        ...
        {
            snprintf(temp_string, 60, TESTRUNNER_LABEL " %zu.\t\t ~~@     Cream      @~~\n", counter);
        }
        if (testcase.pile_array[i] == Fruit)
        {
            snprintf(temp_string, 60, TESTRUNNER_LABEL " %zu.\t\t •¤0     Fruit      0¤•\n", counter);
        }
        if (testcase.pile_array[i] == ActualPancake)
        {
            snprintf(temp_string, 60, TESTRUNNER_LABEL " %zu.\t\t === Actual Pancake ===\n", counter);
        }
        counter++;
...
int main(int argc, char const *argv[])
{
                                                                ...
                                                                "den gleichen Wert haben."))
    {
        for (int i = 0; i < TESTS_2F; i++)
        {
            if (begin_testcase(&tester))
            {
                char *args = render_rgbs(exercise_2f[i].x, exercise_2f[i].y);
                assert_bool(&tester, exercise_2f[i].equal_same,
                            point_to_equal_values_struct(&(exercise_2f[i].x), &(exercise_2f[i].y)), args);
                free(args);
            }
        }
    }
            ...
            if (begin_testcase(&tester))
            {
                if (exercise_2g[i].equal_same)
                {
                    ...
                    assert_bool(&tester, exercise_2g[i].equal_same,
                                point_to_same_values_struct(&(exercise_2g[i].x), &(exercise_2g[i].x)), args);
                    free(args);
                }
                else
                {
                    char *args = render_rgbs(exercise_2f[i].x, exercise_2f[i].y);
                    assert_bool(&tester, exercise_2g[i].equal_same,
                                point_to_same_values_struct(&(exercise_2g[i].x), &(exercise_2g[i].y)), args);
                    free(args);
                }
            }
        }
        ...
        for (int i = 0; i < TESTS_3F; i++)
        {
            if (begin_testcase(&tester))
            {
                ...
                assert_bool(&tester, exercise_3f[i].equal_same,
                            point_to_equal_values_field(&(exercise_3f[i].x), &(exercise_3f[i].y)), args);
                free(args);
            }
        }
    }
            ...
            if (begin_testcase(&tester))
            {
                if (exercise_3g[i].equal_same)
                {
                    ...
                    assert_bool(&tester, exercise_3g[i].equal_same,
                                point_to_same_values_field(&(exercise_3g[i].x), &(exercise_3g[i].x)), args);
                    free(args);
                }
                else
                {
                    char *args = render_rgbs(exercise_2f[i].x, exercise_2f[i].y);
                    assert_bool(&tester, exercise_3g[i].equal_same,
                                point_to_same_values_field(&(exercise_3g[i].x), &(exercise_3g[i].y)), args);
                    free(args);
                }
            }
        }
    ...
    /* Aufgabe 5a */
    if (begin_exercise(&tester, "starts_with_fruit", "Aufgabe 5a: Überprüfen ob erste Schicht aus Früchten ist."))
    {
        for (int i = 0; i < TESTS_5AB; i++)
        {
            if (begin_testcase(&tester))
            {
                ...
                assert_bool(&tester, exercise_5ab[i].pile_array[0] == Fruit, starts_with_fruit(*pile), args);
                free(args);
                eat_pile(pile);
            }
        }
    ...
    /* Aufgabe 5b */
    if (begin_exercise(&tester, "ends_with_fruit", "Aufgabe 5b: Überprüfen ob letzte Schicht aus Früchten ist."))
    {
        for (int i = 0; i < TESTS_5AB; i++)
        {
            if (begin_testcase(&tester))
            {
                char *args = render_exercises_5(exercise_5ab[i]);
                PileOfPancakes *pile = create_pile(exercise_5ab[i].pile_array, exercise_5ab[i].number_of_layers);
                assert_bool(&tester, exercise_5ab[i].pile_array[exercise_5ab[i].number_of_layers - 1] == Fruit, ends_with_fruit(*pile), args);
                free(args);
                eat_pile(pile);
            }
        }
    ...
    /* Aufgabe 5c */
    if (begin_exercise(&tester, "count_fruit_layers", "Aufgabe 5c: Die Anzahl an Fruchtschichten eines Pancake-Piles zählen."))
    {
        for (int i = 0; i < TESTS_5AB; i++)
        {
            if (begin_testcase(&tester))
